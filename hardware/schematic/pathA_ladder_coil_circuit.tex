...
%
% Terminal names and internal topology are taken from the
% manufacturer's own wiring diagram in the listing gallery, so the
% earlier "meter it and find out" hedge is gone: the relay switches
% L, and the N pair is a common bus.
% ============================================================
...
\node[legendbox, anchor=north west, text width=10.4cm] at (0,-5.8) {%
  ...
  receiver between the seal-in and the coil --- that is the Path B
  position and it assumes a dry relay module the project does not own.
};
